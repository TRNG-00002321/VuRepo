...
public class ExpenseServiceImpl implements ExpenseService {
    ...
    public void rejectService(int managerId) {
        ...
        if(!expenses.isEmpty()){
            ...
            printExpenses(expenses);
            System.out.println("Enter a valid expense ID to be rejected");
            int expenseId = sc.nextInt();
            sc.nextLine();
            System.out.print("Enter comment: ");
            String rejectComment = sc.nextLine();
            dao.rejectExpense(expenseId, managerId, rejectComment);
            System.out.println("Successfully rejected the expense");
            System.out.println();
        }
        else{
    ...
    public void printExpenses(List<Expense> expenses){
        ...
        System.out.printf("%-10s %-8s %-10s %-25s %-12s %-10s%n",
                "ExpID", "UserID", "Amount", "Description", "Date", "Status");
        System.out.println("--------------------------------------------------------------------------------");

        for (Expense e : expenses) {
            System.out.printf("%-10d %-8d %-10s %-25s %-12s %-10s%n",
                    e.getExpenseId(),
                    e.getUserId(),
                    "$" + e.getAmount(),
